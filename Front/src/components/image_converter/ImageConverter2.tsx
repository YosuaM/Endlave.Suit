...
export default function ImageConverter() {
  ...
  const handleFormatChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    const newFormat = e.target.value as 'jpeg' | 'png' | 'webp' | 'avif';
    setFormat(newFormat);
    if (file) {
      convertOnly(file);
    }
  ...
  const moveSlider = useCallback((clientX: number) => {
    if (!containerRef.current || !clipRef.current || !dividerRef.current) return;
              ...
              <div className="relative w-full h-full">
                ...
                <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 bg-gray-800/80 backdrop-blur-md rounded-md px-6 py-3 flex flex-col gap-3 items-center shadow-lg w-[280px] text-white">
                  ...
                  <select
                    ...
                    className="border border-white bg-transparent text-white p-2 rounded w-full appearance-none"
                  >
                ...
                <div className="absolute bottom-2 left-4 bg-gray-800/80 text-white px-3 py-1 rounded text-xs">
                  Converted: {format.toUpperCase()} — {convertedBlob ? formatFileSize(convertedBlob.size) : '0B'}
                </div>
                <div className="absolute bottom-2 right-4 bg-gray-800/80 text-white px-3 py-1 rounded text-xs">
                  Original: {file?.type.split('/')[1].toUpperCase()} — {file ? formatFileSize(file.size) : '0B'}
                </div>
              </div>
            </div>
          </div>
